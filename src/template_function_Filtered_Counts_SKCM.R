Filtered_Counts_SKCM <- function(broad_skcm_normalized_genelevel, TCGA_SKCM_harmonized_clinical_data) {
    ...
    if (Use_Group_Based_Filtering == TRUE) {
        rownames(df) <- df[,gene_names_column]
        df[,gene_names_column] <- NULL
        
        if(use_cpm_counts_to_filter == TRUE){
            counts <- edgeR::cpm(df) >  Minimum_Count_Value_to_be_Considered_Nonzero # boolean matrix
        }else{
            counts <- as.matrix(df) > Minimum_Count_Value_to_be_Considered_Nonzero
        }
        ...
        df.filt <- df[tcounts.keep, ]
        df.filt %>% rownames_to_column(gene_names_column) -> df.filt
    } else {
        if (use_cpm_counts_to_filter == TRUE){
            df$isexpr1 <- rowSums(edgeR::cpm(as.matrix(df[, -1])) > Minimum_Count_Value_to_be_Considered_Nonzero) >= Minimum_Number_of_Samples_with_Nonzero_Counts_in_Total
        } else {
            df$isexpr1 <- rowSums(as.matrix(df[, -1]) > Minimum_Count_Value_to_be_Considered_Nonzero) >= Minimum_Number_of_Samples_with_Nonzero_Counts_in_Total
        }
            df.filt <- as.data.frame(df[df$isexpr1, ])
    }

    colnames(df.filt)[colnames(df.filt)==gene_names_column] <- "Gene"
    print(paste0("Number of genes after filtering: ", nrow(df.filt)))

    ######## Start PCA ###############

    edf <- log(edgeR::cpm(as.matrix(df.filt[,samples_to_include]+0.5)))
    ...
    g <- ggplot(pca.df, aes(x=xdata, y=ydata)) +
        ...
        theme(legend.title=element_blank()) +
        theme(legend.position=legend_position_for_PCA) +
        geom_point(aes(color=group,text=sample), size=point_size) +
        theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
            panel.background = element_blank()) +
        scale_colour_manual(values = colorval) +
        xlab(pc.x.lab) + ylab(pc.y.lab)    
    ...
    png(
      filename="Filtered_Counts_SKCM.png",
      width=imageWidth,
      height=imageHeight,
      units="px",
      pointsize=4,
...
print("template_function_Filtered_Counts_SKCM.R #########################################################################")
library(plotly);library(ggplot2);library(jsonlite);
var_broad_skcm_normalized_genelevel<-readRDS("var_broad_skcm_normalized_genelevel.rds")
var_TCGA_SKCM_harmonized_clinical_data<-readRDS("var_TCGA_SKCM_harmonized_clinical_data.rds")
invisible(graphics.off())
var_Filtered_Counts_SKCM<-Filtered_Counts_SKCM(var_broad_skcm_normalized_genelevel,var_TCGA_SKCM_harmonized_clinical_data)
invisible(graphics.off())
saveRDS(var_Filtered_Counts_SKCM,"var_Filtered_Counts_SKCM.rds")
